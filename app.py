...
#1 Livrările cu Numele Curierilor și Descrierea Pachetelor
...
#2 Clienții și Detaliile Pachetelor Lor
...
#3 Pachetele cu Ultimele Actualizări ale Locației
...
#4 Clienții și Suma Totală Facturată:
...
#5 Curierii și Numărul de Livrări Finalizate:
...
#6 Livrările Inclusiv Numele Clienților și Statusul Livrării:
...
#7 Pachete cu Numele și Detaliile de Contact ale Clienților:
...
#1 Clienți cu Facturare Peste Media: 
...
#2 Curieri cu Cel Mai Mare Număr de Livrări în Luna Curentă:
...
#3 Clienți cu Livrări în Mai Mult de un Oraș:
...
#4 Clienți Care Au Trimis un Pachet cu Greutate Peste Media:
...
#5 Clienți Care Au Trimis Pachete de Peste 5 kg în 2024:
...
#6returna numele și emailul clienților care au avut cel puțin o livrare întârziată, 
#împreună cu numărul total de livrări întârziate și ratingul mediu al serviciului de curierat.
@app.route('/customers_late_deliveries')
def customers_late_deliveries():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("""
        SELECT 
            ...
            (
                SELECT COUNT(*)
                FROM dbo.Deliveries del
                INNER JOIN dbo.Packages p ON del.PackageID = p.PackageID
                WHERE p.CustomerID = c.CustomerID 
                      AND del.ActualDeliveryDate > del.ExpectedDeliveryDate
            ) AS LateDeliveries,
            (
                SELECT AVG(r.Rating)
                FROM dbo.Ratings r
    ...
    cursor.close()
    conn.close()
    return render_template('customers_late_deliveries.html', results=results)
